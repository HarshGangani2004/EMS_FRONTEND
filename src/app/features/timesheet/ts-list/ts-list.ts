...
@Component({
  ...
  templateUrl: './ts-list.html',
  styleUrls: ['./ts-list.css']
})
export class TsListComponent implements OnInit {
  ...
  // =========================
  loadProjects() {
  if (!this.permissionService.has('project.view')) {
    this.projects = [];
    return;
  }
  ...
  this.http
    ...
    .subscribe({
      ...
      error: err => console.error('PROJECT API ERROR', err)
    });
}
  ...
  load() {
    ...
    this.service.getMyTs(this.page, this.pageSize, this.filter)
      .subscribe({
        next: (res: any) => {
  ...
  edit(id: number) {
    this.router.navigate(['/timesheet/update', id]);
  }
 ...
 hasanyActionPermission(): boolean {
  return this.permissionService.has('timesheet.update') ||
         this.permissionService.has('timesheet.delete') ||
        this.permissionService.has('timesheet. view')||
          this.permissionService.has('timesheet.create');
  }
